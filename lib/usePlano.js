  ...
  useEffect(() => {
    async function carregar() {
      ...
      const { data: perfil } = await supabase
        ...
        .single()
      if (perfil) {
        if (perfil.role === 'admin') { setPlano('business'); setAtivo(true); setLoading(false); return }
        const expirou = perfil.plano_expira ? new Date(perfil.plano_expira) < new Date() : false
